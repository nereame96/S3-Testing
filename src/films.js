...
function hoursToMinutes(array) {
  const durationNoText = array.movie.duration.replace('h', '').replace('min', '')
  const [hours, minutes] = durationNoText.movie.duration.split(' ')
  const calculateMin =  (+hours * 60) + (+minutes)  
  const result = 
}
